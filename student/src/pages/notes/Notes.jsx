// ✅ src/pages/notes/Notes.jsx (ULTRA PREMIUM OVERHAUL)
import React, { useContext, useMemo, useState, useEffect, useCallback, useRef } from "react";
import { useNavigate, Link } from "react-router-dom";
import { motion, AnimatePresence, useReducedMotion } from "framer-motion";
import {
  ChevronRight, Search, Sparkles, BookOpen, StickyNote, Pin, Clock, Tag,
  Download, Filter, ArrowLeft, RefreshCcw, X, SlidersHorizontal, Layers
} from "lucide-react";
...
/** ✅ Ultra 3D Note Card with Mouse Spotlight */
function NoteCard3D({ isDark, accent, children, idx = 0 }) {
...
export default function Notes() {
  ...
  const t = useMemo(() => {
    const en = {
      crumbHome: "Home",
      crumbHere: "Notes Library",
      title: "Notes Library",
      subtitle: "Topic-wise PDFs, revision sheets, diagrams & cheat-sheets — beautifully organized, lightning fast.",
      searchPH: "Search notes, tags, or topics...",
      pinned: "Pinned",
      tabs: { all: "All Notes", pinned: "Pinned", recent: "Recent" },
      minutes: "mins read",
    ...
    const hi = {
      crumbHome: "Home",
      crumbHere: "Notes लाइब्रेरी",
      title: "Notes लाइब्रेरी",
      subtitle: "टॉपिक-वाइज़ PDFs, रिविज़न शीट्स, डायग्राम्स व चीट-शीट्स — सुंदर और तेज़।",
      searchPH: "नोट्स, टैग या टॉपिक खोजें...",
      pinned: "पिन्ड",
      tabs: { all: "सभी नोट्स", pinned: "पिन्ड", recent: "रीसेंट" },
      minutes: "मिनट रीड",
      download: "PDF डाउनलोड",
      reset: "फिल्टर रीसेट",
      refresh: "रिफ्रेश",
      emptyTitle: "कोई नोट नहीं मिला",
  ...
  const LS_KEY = "hl_notes_state_v4";
  ...
  useEffect(() => {
    try {
      localStorage.setItem(LS_KEY, JSON.stringify(notesState));
    } catch {}
  }, [notesState]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (q) qs.set("q", q);
      if (cat !== "all") qs.set("tag", cat);

      const res = await fetch(`${API_BASE}/api/notes/student?${qs.toString()}`);
      ...
      if (items.length === 0) {
          setItems([
              { id: "1", title: "Advanced Quantum Mechanics", desc: "Comprehensive guide to quantum states and entanglement.", tag: "Physics", mins: 12 },
              { id: "2", title: "Organic Chemistry Cheat Sheet", desc: "Reaction mechanisms and functional groups simplified.", tag: "Chemistry", mins: 8 },
              { id: "3", title: "Calculus III Integration", desc: "Multiple integrals and vector calculus theorem sheets.", tag: "Math", mins: 15 },
              { id: "4", title: "Cellular Biology Diagrams", desc: "High-res diagrams of cell organelles and their functions.", tag: "Biology", mins: 5 },
          ]);
      }
    } finally {
      setLoading(false);
  ...
  const categories = useMemo(() => {
    const uniq = new Set((items || []).map((x) => x.tag).filter(Boolean));
  ...
  const filtered = useMemo(() => {
    ...
    arr = arr.map((n) => ({
      ...n,
      pinned: !!notesState?.pinned?.[n.id],
      lastOpened: notesState?.lastOpened?.[n.id] ?? 0,
      downloadsLocal: notesState?.downloads?.[n.id] ?? 0,
    }));
    ...
    if (qq) {
      arr = arr.filter((x) => {
        ...
        return title.includes(qq) || desc.includes(qq) || tag.includes(qq);
      });
    }
    ...
    return arr;
  }, [notes, notesState, tab, q, language]);
      ...
      <div className={container}>
        ...
        <div className="mt-12 text-center relative">
          <div className="flex items-center justify-center">
            <Badge isDark={isDark}>{language === "hi" ? "प्रीमियम PDFs • डायग्राम्स" : "Premium PDFs • Cheat-Sheets"}</Badge>
          </div>
          ...
          >
            {t.title}{" "}
            <span className="bg-gradient-to-tr from-emerald-400 via-sky-400 to-fuchsia-500 bg-clip-text text-transparent inline-block hover:scale-110 transition-transform cursor-default">
              ✨
            </span>
          </motion.h1>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <AnimatePresence mode="popLayout">
            {filtered.map((n, idx) => {
              const title = pickText(n.title, language) || "Untitled Note";
              const desc = pickText(n.desc, language) || "No description provided.";
              const mins = Number(n.mins || 5);
              ...
              return (
                <NoteCard3D
                  key={n.id}
                  isDark={isDark}
                  accent={"from-emerald-400 via-sky-400 to-fuchsia-500"}
                  idx={idx}
                >
                    ...
                    <div className="flex items-center gap-3">
                      <motion.div
                        className={cn(
                          ...
                          isDark
                            ? "bg-slate-900/80 border-white/20 shadow-[0_20px_40px_rgba(0,0,0,0.4)] group-hover:shadow-[0_20px_40px_rgba(14,165,233,0.2)]"
                            : "bg-white/90 border-white shadow-[0_20px_40px_rgba(2,6,23,0.1)] group-hover:shadow-[0_20px_40px_rgba(14,165,233,0.2)]"
                        )}
                        whileHover={reduce ? {} : { rotate: 8, scale: 1.05 }}
                      >
                        <BookOpen className={cn("h-6 w-6", isDark ? "text-white" : "text-sky-600")} />
                      </motion.div>
                  ...
                  <motion.button
                    ...
                    onClick={() => {
                      markOpened(n.id);
                      window.open(`${API_BASE}/api/notes/download/${n.id}`, "_blank", "noopener,noreferrer");
                    }}
                    className={cn(
                  ...
                  >
                    <div className="absolute inset-0 bg-gradient-to-r from-emerald-400 via-sky-400 to-fuchsia-500 opacity-0 group-hover/btn:opacity-20 transition-opacity duration-500" />
